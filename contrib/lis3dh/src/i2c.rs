...
use crate::{register::*, Configuration, Error, Lis3dhImpl, SlaveAddr};
...
where
    I2C: WriteRead<Error = EI2C> + Write<Error = EI2C>,
    EI2C: Debug,
{
    ...
    pub fn new(
        ...
        address: SlaveAddr,
        conf: Configuration,
    ) -> Result<Self, Error<EI2C, Infallible>> {
...
impl<I2C, EI2C> crate::Lis3dhImpl for Lis3dh<I2C>
where
    I2C: WriteRead<Error = EI2C> + Write<Error = EI2C>,
    EI2C: Debug,
{
    ...
    fn read_register(
        ...
        self.i2c
            .write_read(self.address, &mut [register.addr()], &mut data)
            .map_err(Error::Bus)
            .and(Ok(data[0]))
    }
}

impl<I2C, EI2C> RawAccelerometer<I16x3> for Lis3dh<I2C>
where
    I2C: WriteRead<Error = EI2C> + Write<Error = EI2C>,
    EI2C: Debug,
{
    type Error = Error<EI2C, Infallible>;
...
impl<I2C, EI2C> Accelerometer for Lis3dh<I2C>
where
    I2C: WriteRead<Error = EI2C> + Write<Error = EI2C>,
    EI2C: Debug,
{
