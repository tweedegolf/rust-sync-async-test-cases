...
pub trait Lis3dhImpl {
    ...
    fn init(&mut self, conf: Configuration) -> Result<(), Error<Self::BusError, Self::PinError>> {
    ...
    fn configure_irq_src<I: Interrupt>(
        &mut self,
        _int: I,
        ...
        self.write_register(I::cfg_reg(), irq_src.bits())
    }

    /// Configure a duration threshold for an interrupt
    fn configure_irq_duration<I: Interrupt>(
        &mut self,
        _int: I,
        duration: u8,
    ) -> Result<(), Error<Self::BusError, Self::PinError>> {
        self.write_register(I::duration_reg(), duration)
    }

    /// Configure a magnitude threshold for an interrupt
    fn configure_irq_threshold<I: Interrupt>(
        &mut self,
        _int: I,
        threshold: u8,
    ) -> Result<(), Error<Self::BusError, Self::PinError>> {
        self.write_register(I::ths_reg(), threshold)
    }

    /// Get interrupt source status
    fn get_irq_src<I: Interrupt>(
        &mut self,
        _int: I,
    ) -> Result<InterruptSource, Error<Self::BusError, Self::PinError>> {
        let irq_src = self.read_register(I::src_reg())?;
        Ok(InterruptSource {
            and_or_combination: irq_src & (1 << 7) != 0,
            interrupt_active: irq_src & (1 << 6) != 0,
    ...
    fn configure_act(
        ...
        self.write_register(Register::ACT_THS, ths & 0x7F)?;
        self.write_register(Register::ACT_DUR, dur)
    }
